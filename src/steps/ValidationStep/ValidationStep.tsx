...
export const ValidationStep = <T extends string>({ initialData, file, onBack, ...restProps }: Props<T>) => {
  const { translations, onClose, onSubmit, rowHook, tableHook, ...restRsi } = useRsi<T>()
  // override original fields with fields from props
  const fields = restProps.fields ?? restRsi.fields
